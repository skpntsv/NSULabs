...
// TODO ЭТО ВСЁ НАДО НАПИСАТЬ

Как в многопоточной программе работают сигналы

есть ли гарантия что сигинт будет обрабатываться (нет, т.к. первый поток может перезаписать его обработчик)

чем пользовались для обеспечения работы сигналов (маски)

у второго потока через сигнал прописал что через сигинт есть сигхендлер

на третий создал макс куда добавил сигквит и добавил чтоб ждали этот сигнал

сигвэйт приостанавливает поток до тех пор пока мы не получим один из сигналов в этой маске

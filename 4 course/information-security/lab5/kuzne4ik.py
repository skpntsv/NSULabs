...
def xor_func(first_value, second_value, in_code = 16):
    input1AsInteger = int(first_value, in_code)
    input2AsInteger = int(second_value, in_code)
    result = input1AsInteger ^ input2AsInteger
    resultAsHex = hex(result)
    resultAsHex = resultAsHex.upper()
    resultAsHex = resultAsHex[2:]
    if len(resultAsHex) != len(first_value):
        for i in range(len(first_value) - len(resultAsHex)):

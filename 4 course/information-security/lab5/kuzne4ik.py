...
# XOR для двух строк
def XOR(str1, str2, base=16):
    int1, int2 = int(str1, base), int(str2, base)
    xor_result = int1 ^ int2
    hex_result = hex(xor_result)[2:].upper()
    return hex_result.zfill(len(str1))
...
def linear_transformation(num, move = Direction.STRAIGHT):
    ...
    for i in range(16):
        ...
        if len(galua) != 1:
            for j in range(len(galua) - 1):
                ...
                galua_num = int(XOR(str(galua_num), str(galua[j + 1]), base = 10), 16) % 256
...
def nonlinear_transformation(num, move = Direction.STRAIGHT):
    for i in range(16):
        if move == Direction.REVERSE:
...
def decrypt(text, K):
    ...
    for j in textArray:
        ...
        for i in range(9, 0, -1):
            textDecrypted = S(L(X(textDecrypted, K[i]), Direction.REVERSE), Direction.REVERSE)
...
key = getKeys(password)
print(f"Сгенерированные ключи: {key}")
